...
for(i in 2:length(yEar)){ cat(yR[i],", ")
	if(i<7){ ## 2008-2013
		...
		#for(i0 in c("uppl", "sleep", "replacement", "anaphylaxis", "sugar", "skin", "sea ", "eyedrops", "lotion", "juice", "feed", "energy", "leg ")){i1 = c(i1,grep(i0,x0[,5]))}
		#x0 = x0[-unique(i1),]
...
## remap medication history to per patients basis 20220421
		...
		for(i0 in c("shake", "juice", "water", "factor")){x0[grep(i0,x0[,2]),2] = x0[grep(i0,x0[,3]),3] = ""}
...
## NTM multi-entries for repeated pwCF entries
		xN = cf425[[p[grep("NTM", p)]]]
		xN = xN[,-which(colnames(xN) %in% c("ntmculturedate","ntmculturetype"))]
		xN[,-1] = xN[,-1] %% 2 # 1 = presence, 2 = absence from data
		xNp = unique(xN[,1])
		xN0 = as.data.frame(matrix(0,nr=length(xNp),nc=ncol(xN)))
		colnames(xN0) = colnames(xN); xN0[,1] = xNp
		for(i0 in 1:nrow(xN0)){xN0[i0,-1] = ifelse(colSums(xN[which(xN[,1]==xN0[i0,1]),-1])>0,1,0)}
## merge reformatted medication histories to remaining data
		x0 = merge(x1, xN0, all=T)
...
cat("separate direct/indirect reference columns:",date(),"\n")
...
cat("data segregation completed:",date(),"\n")
